...
// src/hooks/useBscEnsName.ts
import { useEffect as useEffect5, useState as useState2 } from "react";
import { useProvider as useProvider2 } from "wagmi";
import SID, { getSidAddress } from "@siddomains/sidjs";
function useBscEnsName(chainId, address) {
  const [ensName, setEnsName] = useState2();
  const provider = useProvider2();
  const fetchEnsName = async () => {
    const ENS = SID.default;
    const ens = new ENS({ provider, sidAddress: getSidAddress("56") });
    const res = await ens.getName(address);
    setEnsName(res.name || void 0);
  };
  useEffect5(() => {
    console.log("chainId--chainId", chainId.toString());
    if (address && chainId.toString() === "56") {
      fetchEnsName();
    }
...
function useMainnetEnsName(address) {
  ...
  const { ensName: bscEnsName } = useBscEnsName(curChainId, address);
  console.log("bscEnsName", bscEnsName);
  console.log("ensName", ensName);
  return ensName || bscEnsName;
}
...
import React10, { createContext as createContext4, useContext as useContext4, useEffect as useEffect6, useState as useState3 } from "react";
import { useAccount as useAccount3, useProvider as useProvider3 } from "wagmi";
...
function TransactionStoreProvider({
  ...
  const provider = useProvider3();
  const { address } = useAccount3();
  const chainId = useChainId2();
  const [store] = useState3(() => storeSingleton != null ? storeSingleton : storeSingleton = createTransactionStore({ provider }));
  useEffect6(() => {
...
function useRecentTransactions() {
  ...
  const chainId = useChainId2();
  const [transactions, setTransactions] = useState4(() => store && address && chainId ? store.getTransactions(address, chainId) : []);
  useEffect7(() => {
...
import React20, { useCallback as useCallback3, useEffect as useEffect10, useRef as useRef3 } from "react";
...
function Dialog({ children, onClose, open, titleId }) {
  ...
  const [bodyScrollable, setBodyScrollable] = useState6(true);
  useEffect11(() => {
...
import React32, { useCallback as useCallback6, useContext as useContext8, useEffect as useEffect12, useState as useState7 } from "react";
...
function ChainModal({ onClose, open }) {
  ...
  const [switchingToChain, setSwitchingToChain] = useState8();
...
import React49, { Fragment as Fragment2, useContext as useContext13, useEffect as useEffect16, useState as useState10 } from "react";
...
import React43, { useState as useState9 } from "react";

// src/components/RainbowKitProvider/useCoolMode.ts
import { useContext as useContext11, useEffect as useEffect14, useRef as useRef4 } from "react";
...
import React48, { useContext as useContext12, useEffect as useEffect15 } from "react";
...
function DesktopOptions({ onClose }) {
  ...
  const safari = isSafari();
  const [selectedOptionId, setSelectedOptionId] = useState10();
  ...
  return /* @__PURE__ */ React49.createElement(Box, {
    ...
    size: "14",
    weight: "medium"
  }, "New to wallets?")), /* @__PURE__ */ React49.createElement(Box, {
    alignItems: "center",
    display: "flex",
    flexDirection: "row",
    gap: "4",
...
import React50, { useCallback as useCallback8, useContext as useContext14, useState as useState11 } from "react";
...
function MobileOptions({ onClose }) {
  ...
  let headerBackButtonLink = null;
  const [walletStep, setWalletStep] = useState11("CONNECT" /* Connect */);
  const ios = isIOS();
  switch (walletStep) {
    case "CONNECT" /* Connect */: {
...
function useModalStateValue() {
  const [isModalOpen, setModalOpen] = useState12(false);
  return {
